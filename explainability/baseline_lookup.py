...
class Baseline_Lookup:
    ...
    def __init__(self, dataset_list, node_types, device="cpu"):
        ...
        for node_type in node_types:
            self.nodes_feature_length[node_type] = dataset_list[0][0][
                node_type
            ].x.shape[1]
        ...
        self.node_pos = {}
        for node_type in node_types:
            self.nodes[node_type] = torch.zeros(
                (self.num_nodes[node_type], self.nodes_feature_length[node_type]),
                device=device,
            )
        ...
        for dataset in dataset_list:
            for data in dataset:
                for key in node_types:
                    self.nodes[key][
                        current_node[key] : current_node[key] + data[key].pos.shape[0],
                        :,
                    ] = data[key].x
                    self.node_pos[key][
                        current_node[key] : current_node[key] + data[key].pos.shape[0],
    ...
    def get_k_nearst_neighbors_avg_features(self, node_type, pos, k):
        dist, ind = self.trees[node_type].query(pos, k=k)
        avg = self.nodes[node_type][ind, :].mean(
            axis=1
        )  # axis = 1 for featurewise mean over all neighbors
    ...
    def get_k_nearst_neighbors_avg_features_quantile(
        self, node_type, pos, k, lower_quantile=0.1, upper_quantile=0.9
    ):
        ...
        in_quantile_range = (neighbors > lower_quantiles[:, None, :]) & (
            neighbors < upper_quantiles[:, None, :]
        )
        avg = torch.where(
            in_quantile_range, neighbors, torch.tensor(0.0, device=neighbors.device)
        )
        avg = avg.mean(dim=1)
    ...
    def get_baselines(self, baseline_type, data, k=100):
        ...
        if baseline_type == "zero":
            ...
            baseline_faz = torch.zeros_like(
                data.x_dict["faz"], device=device
            ).unsqueeze(0)

        elif baseline_type == "lookup_quantile":  # get_k_nearst_neighbors_avg_features
            baseline_graph_1 = self.get_k_nearst_neighbors_avg_features_quantile(
                "graph_1", data["graph_1"].pos.cpu().numpy(), k
            ).unsqueeze(0)
            baseline_graph_2 = self.get_k_nearst_neighbors_avg_features_quantile(
                "graph_2", data["graph_2"].pos.cpu().numpy(), k
            ).unsqueeze(0)
            baseline_faz = torch.zeros_like(
                data.x_dict["faz"], device=device
            ).unsqueeze(0)
            ...
            baseline_graph_1 = self.get_k_nearst_neighbors_median_features(
                "graph_1", data["graph_1"].pos.cpu().numpy(), k
            ).unsqueeze(0)
            baseline_graph_2 = self.get_k_nearst_neighbors_median_features(
                "graph_2", data["graph_2"].pos.cpu().numpy(), k
            ).unsqueeze(0)
            baseline_faz = torch.zeros_like(
                data.x_dict["faz"], device=device
            ...
            baseline_graph_1 = self.get_k_nearst_neighbors_avg_features(
                "graph_1", data["graph_1"].pos.cpu().numpy(), k
            ).unsqueeze(0)
            baseline_graph_2 = self.get_k_nearst_neighbors_avg_features(
                "graph_2", data["graph_2"].pos.cpu().numpy(), k
            ).unsqueeze(0)
            baseline_faz = torch.zeros_like(
                data.x_dict["faz"], device=device
