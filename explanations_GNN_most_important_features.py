...
if not model_config["parameters"]["faz_node"] or (
    not model_config["parameters"]["aggr_faz"]
    and not model_config["parameters"]["hetero_conns"]
    and not model_config["parameters"]["start_rep"]
):
    print("Deleting FAZ")
...
clf = graph_classifier.graphClassifierHetero_94d26db(
    model, init_loss, lr=0.01, weight_decay=0.001, smooth_label_loss=True
)
# load the state dict
loss, y_prob_train, y_true_train, _ = clf.train(
    train_loader
...
for idx, data in enumerate(gnnexp_loader):
    ...
    prediction = explainer.get_prediction(
        data.x_dict, data.edge_index_dict, batch_dict=data.batch_dict
    )
    ...
    for key in explanation.node_mask_dict.keys():
        if data.y[0] == target.item() == 1:
            if most_important_features_npdr[key] is None:
                most_important_features_npdr[key] = (
                    work_dict[key].sum(dim=0).cpu().numpy()
                )
            else:
                most_important_features_npdr[key] += (
                    work_dict[key].sum(dim=0).cpu().numpy()
            ...
            if most_important_features_pdr[key] is None:
                most_important_features_pdr[key] = (
                    work_dict[key].sum(dim=0).cpu().numpy()
                )
            else:
                most_important_features_pdr[key] += (
...
feature_importance_pdr = global_feature_importance.GlobalFeatureImportance(
    most_important_features_pdr, features_label_dict, "All Correct PDR Predictions"
)
feature_importance_pdr.combined_feature_plot(num_features=10)

feature_importance_npdr = global_feature_importance.GlobalFeatureImportance(
    most_important_features_npdr, features_label_dict, "All Correct NPDR Predictions"
)
