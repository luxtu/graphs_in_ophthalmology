...
#
    ...
    def plot_graph_2D_faz(self, het_graph , edges= True, ax = None, pred_val_dict = None, path = None):
        # create a function that returns a plot of the graph in 2D
        # use the node positions and the edge indices to plot the graph
        ...
        het_graph1_pos = het_graph[self.graph_1_name].pos.cpu().detach().numpy()
        het_graph2_pos = het_graph[self.graph_2_name].pos.cpu().detach().numpy()
        het_graph3_pos = het_graph[self.graph_3_name].pos.cpu().detach().numpy()
...
#
            ...
            for edge in het_graph[self.graph_3_name, 'to', self.graph_2_name].edge_index.cpu().detach().numpy().T:
                x_pos = (het_graph3_pos[edge[0],1], het_graph2_pos[edge[1],1])
                y_pos = (het_graph3_pos[edge[0],0], het_graph2_pos[edge[1],0])
#
                ax.plot(x_pos, y_pos, linewidth=1, alpha=0.5, c= "yellow", zorder = 1)

            for edge in het_graph[self.graph_1_name, 'to', self.graph_3_name].edge_index.cpu().detach().numpy().T:
                x_pos = (het_graph1_pos[edge[0],1], het_graph3_pos[edge[1],1])
                y_pos = (het_graph1_pos[edge[0],0], het_graph3_pos[edge[1],0])

                ax.plot(x_pos, y_pos, linewidth=1, alpha=0.5, c= "purple", zorder = 1)
        if pred_val_dict is not None:
