...
class Heterogeneous_GNN(torch.nn.Module):
    def __init__(
        ...
        for _ in range(self.num_pre_processing_layers):
            ...
            for node_type in self.node_types:
                ...
                if batch_norm:
                    pre_batch_norm_dict[node_type] = torch.nn.BatchNorm1d(
                        hidden_channels
                    )
                else:
        ...
        for _ in range(self.num_post_processing_layers):
            ...
            for node_type in self.node_types:
                ...
                if batch_norm:
                    post_batch_norm_dict[node_type] = torch.nn.BatchNorm1d(
                        hidden_channels
                    )
                else:
        ...
        for _ in range(num_layers):
            if self.faz_node:
                conv = self.conv_gnn_faz(
                    self.hidden_channels,
                    self.conv_aggr,
                    self.homogeneous_conv,
                    self.heterogeneous_conv,
                )
            else:
                conv = self.conv_gnn(
                    self.hidden_channels,
            ...
            if self.conv_aggr == "cat":
                self.cat_comps.append(
                    HeteroDictLinear(-1, hidden_channels, types=self.node_types)
                )
    ...
    def forward(self, x_dict, edge_index_dict, batch_dict, **kwargs):
                ...
                if isinstance(self.aggregation_mode, list):
                    for j in range(len(self.aggregation_mode)):
                        ...
                        if self.aggregation_mode[j] == global_add_pool:
                            start_representations.append(
                                self.aggregation_mode[j](x_dict[key], batch_dict[key])
                            )  # /1000
                        else:
                            start_representations.append(
                                self.aggregation_mode[j](x_dict[key], batch_dict[key])
                    ...
                    start_representations.append(
                        self.aggregation_mode(x_dict[key], batch_dict[key])
                    )
        ...
        for i in range(len(self.pre_processing_lin_layers)):
            for node_type in self.node_types:
                x[node_type] = self.activation(
                    self.pre_processing_batch_norm[i][node_type](
                        self.pre_processing_lin_layers[i][node_type](x_dict[node_type])
                    )
                )
        ...
        for i in range(len(self.post_processing_lin_layers)):
            for node_type in self.node_types:
                try:
                    x[node_type] = self.post_processing_batch_norm[i][node_type](
                        self.post_processing_lin_layers[i][node_type](x[node_type])
                    )
                except KeyError:
                    pass
        ...
        for key in aggr_keys:
            if isinstance(self.aggregation_mode, list):
                for j in range(len(self.aggregation_mode)):
                    # check if the aggregation mode is global_add_pool
                    if self.aggregation_mode[j] == global_add_pool:
                        type_specific_representations.append(
                            self.aggregation_mode[j](x[key], batch_dict[key])
                        )  # /1000
                    else:
                        type_specific_representations.append(
                            self.aggregation_mode[j](x[key], batch_dict[key])
                ...
                type_specific_representations.append(
                    self.aggregation_mode(x[key], batch_dict[key])
                )
    ...
    def conv_gnn(
        self, hidden_channels, conv_aggr, homogeneous_conv, heterogeneous_conv
    ):
        if self.hetero_conns:
            conv = HeteroConv(
                {
                    ("graph_1", "to", "graph_1"): homogeneous_conv(
                        -1, hidden_channels
                    ),  # , aggr = ["mean", "std", "max"],
                    ("graph_2", "to", "graph_2"): homogeneous_conv(-1, hidden_channels),
                    ("graph_1", "to", "graph_2"): heterogeneous_conv(
                        (-1, -1), hidden_channels, add_self_loops=False
                    ),
                    ("graph_2", "rev_to", "graph_1"): heterogeneous_conv(
                        (-1, -1), hidden_channels, add_self_loops=False
                    ),
                },
                aggr=conv_aggr,
            ...
            conv = HeteroConv(
                {
                    ...
                    ("graph_1", "to", "faz"): heterogeneous_conv(
                        (-1, -1), hidden_channels, add_self_loops=False
                    ),
                    ("faz", "to", "graph_2"): heterogeneous_conv(
                        (-1, -1), hidden_channels, add_self_loops=False
                    ),
                    ("graph_2", "rev_to", "graph_1"): heterogeneous_conv(
                        (-1, -1), hidden_channels, add_self_loops=False
                    ...
                    ("faz", "rev_to", "graph_1"): heterogeneous_conv(
                        (-1, -1), hidden_channels, add_self_loops=False
                    ),
                    ("graph_2", "rev_to", "faz"): heterogeneous_conv(
                        (-1, -1), hidden_channels, add_self_loops=False
                    ),
                },
                aggr=conv_aggr,
                ...
                conv = HeteroConv(
                    {
                        ...
                        ("graph_2", "to", "graph_2"): homogeneous_conv(
                            -1, hidden_channels
                        ),
                    },
                    aggr=conv_aggr,
