...
class GNN_Stages(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, num_layers, dropout, aggregation_mode ,node_types, meta_data = None, num_pre_processing_layers = 3, num_post_processing_layers = 3, batch_norm = True, conv_aggr = "cat", hetero_conns = True):
        ...
        self.diseased_gnn = GNN_Binary(hidden_channels, 1, num_layers, dropout, aggregation_mode ,node_types, meta_data = meta_data, num_pre_processing_layers = num_pre_processing_layers, num_post_processing_layers = num_post_processing_layers, batch_norm = batch_norm, conv_aggr = conv_aggr, hetero_conns = hetero_conns)
        self.disease_stage_gnn = GNN_Binary(hidden_channels, 1, num_layers, dropout, aggregation_mode ,node_types, meta_data = meta_data, num_pre_processing_layers = num_pre_processing_layers, num_post_processing_layers = num_post_processing_layers, batch_norm = batch_norm, conv_aggr = conv_aggr, hetero_conns = hetero_conns)



    def forward(self, x_dict, edge_index_dict, batch_dict, grads = False, **kwargs):

        out_diseased = self.diseased_gnn(x_dict, edge_index_dict, batch_dict, grads = grads, **kwargs)
        out_stage = self.disease_stage_gnn(x_dict, edge_index_dict, batch_dict, grads = grads, **kwargs)


        return out_diseased, out_stage
...
class GNN_Binary(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, num_layers, dropout, aggregation_mode ,node_types, meta_data = None, num_pre_processing_layers = 3, num_post_processing_layers = 3, batch_norm = True, conv_aggr = "cat", hetero_conns = True):
        super().__init__()
        torch.manual_seed(1234567)
    ...
    def forward(self, x_dict, edge_index_dict, batch_dict, grads = False, **kwargs):
       ...
       ########################################
        ...
        for i in range(len(self.pre_processing_lin_layers)):
            for node_type in self.node_types:
                x[node_type] = F.leaky_relu(self.pre_processing_batch_norm[i][node_type](self.pre_processing_lin_layers[i][node_type](x_dict[node_type]))) #.relu()
        #########################################
        #########################################
        # gnn convolutions
        for i, conv in enumerate(self.convs): # vs self.convs
            ...
            for node_type in self.node_types:
                ...
                x[node_type] = F.leaky_relu(x[node_type])#.relu()
        ...
        return F.sigmoid(self.lin3(F.leaky_relu(self.lin2(F.leaky_relu(self.lin1(x))))))
