...
class HeteroGNNExplainer(ExplainerAlgorithm):
    ...
    def forward(
        ...
        **kwargs,
        ...
        node_mask_dict = self._post_process_custom_dict(
            self.node_mask_dict,
            self.hard_node_mask_dict,
            self.node_feature_mask_dict,
            self.hard_node_feature_mask_dict,
            apply_sigmoid=True,
        )
        ...
        **kwargs,
        ...
        for i in range(self.epochs):
            ...
            for key in x_dict.keys():
                ...
                # multiply with the node mask and the node feature mask to get a (N,F) tensor

                if self.node_mask_dict is None:
                    h[key] = x
                else:
                    work_mask = torch.matmul(self.node_mask_dict[key], self.node_feature_mask_dict[key])
                    h[key] = x * work_mask.sigmoid()
            ...
            loss = self._loss(y_hat, y, verbose)
            ...
            if i == 0 and self.node_mask_dict is not None:
                ...
                for key in self.node_mask_dict.keys():
                    ...
                    self.hard_node_feature_mask_dict[key] = self.node_feature_mask_dict[key].grad != 0.0
    ...
    def _initialize_masks(self, x_dict: dict, edge_index_dict: dict):
        ...
        for key in x_dict.keys():
            ...
            if node_mask_type is None:
                ...
                self.node_mask_dict[key] = Parameter(torch.randn(N, 1, device=device) * std)
                self.node_feature_mask_dict[key] = Parameter(torch.randn(1, F, device=device) * std)
    ...
    # loss is fine
    def _loss(self, y_hat: Tensor, y: Tensor, verbose: bool) -> Tensor:
        ...
        if self.hard_edge_mask_dict:
            ...
            for key in self.edge_mask_dict.keys():
                ...
                edge_loss_p1 = self.coeffs['edge_size'] * edge_reduce(m)
                if verbose:
                    print("edge_loss_p1", edge_loss_p1)
                loss = loss + edge_loss_p1
                ent = -m * torch.log(m + self.coeffs['EPS']) - (
                    1 - m) * torch.log(1 - m + self.coeffs['EPS'])
                edge_loss_p2 = self.coeffs['edge_ent'] * ent.mean()
                if verbose:
                    print("edge_loss_p2", edge_loss_p2)
                loss = loss + edge_loss_p2
        ...
        if self.hard_node_mask_dict:
            ...
            for key in self.node_mask_dict.keys():
                ...
                node_loss_p1 = self.coeffs['node_feat_size'] * node_reduce(m)
                if verbose:
                    print("node_loss_p1 node mask", node_loss_p1)
                loss = loss + node_loss_p1
                ent = -m * torch.log(m + self.coeffs['EPS']) - (
                    1 - m) * torch.log(1 - m + self.coeffs['EPS'])
                node_loss_p2 = self.coeffs['node_feat_ent'] * ent.mean()
                if verbose:
                    print("node_loss_p2 node mask", node_loss_p2)
                loss = loss + node_loss_p2

                # same for the node feature mask
                m = self.node_feature_mask_dict[key][self.hard_node_feature_mask_dict[key]].sigmoid()
                node_reduce = getattr(torch, self.coeffs['node_feat_reduction'])
                node_loss_p1 = self.coeffs['node_feat_size'] * node_reduce(m)
                if verbose:
                    print("node_loss_p1 feature mask", node_loss_p1)
                loss = loss + node_loss_p1
                ent = -m * torch.log(m + self.coeffs['EPS']) - (
                    1 - m) * torch.log(1 - m + self.coeffs['EPS'])
                node_loss_p2 = self.coeffs['node_feat_ent'] * ent.mean()
                if verbose:
                    print("node_loss_p2 feature mask", node_loss_p2)
                loss = loss + node_loss_p2
    ...
    @staticmethod
    def _post_process_custom_dict(
        node_mask: Optional[dict],
        node_hard_mask: Optional[dict] = None,
        node_feature_mask: Optional[dict] = None,
        node_feature_hard_mask: Optional[dict] = None,
        apply_sigmoid: bool = True,
    ) -> Optional[Tensor]:
        r""""Post processes any mask to not include any attributions of
        elements not involved during message passing.
        ...
        for key in node_mask.keys():
            node_mask[key] = node_mask[key].detach()
            node_feature_mask[key] = node_feature_mask[key].detach()
            final_mask[key] = torch.matmul(node_mask[key], node_feature_mask[key])

            if apply_sigmoid:
                final_mask[key] = final_mask[key].sigmoid()


        if node_hard_mask is not None:
            for key in node_hard_mask.keys():
                if node_mask[key].size(0) == node_hard_mask[key].size(0):
                    # multiplicaiton not supported for bool tensors
                    #work_hard_mask = torch.matmul(node_hard_mask[key], node_feature_hard_mask[key])
                    # cast to float
                    work_hard_mask = torch.matmul(node_hard_mask[key].float(), node_feature_hard_mask[key].float())
                    final_mask[key][work_hard_mask > 0.] = 0.
